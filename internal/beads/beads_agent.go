...
// Updates the description's agent_state field, which is the source of truth
// in bd 0.62.0+ (the dedicated agent_state column was removed from bd's API).
func (b *Beads) UpdateAgentState(id string, state string) (retErr error) {
	...
	// Update description's agent_state field directly.
	// bd 0.62.0 removed the `bd agent state` subcommand that previously
	// updated a dedicated database column. bd show --json no longer returns
	// an agent_state column, so GetAgentBead and the daemon health checks
	// both fall back to the description text. Updating the description is
	// the correct and only mechanism in bd 0.62.0+.
...
func (b *Beads) GetAgentBead(id string) (*Issue, *AgentFields, error) {
	...
	fields := ParseAgentFields(issue.Description)
	// Prefer the structured agent_state column when present (legacy bd versions).
	// bd 0.62.0+ no longer populates this column via bd show --json, so
	// description-derived state is authoritative in current installations.
	if issue.AgentState != "" {
